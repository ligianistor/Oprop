...
-----------------OK until here---------------------------------------
{
	 ...
	 assume (forall y:Ref :: (fracRight[y] >= 0.0) );
if (parent[this]!=null) {
fracParent[opp] := fracParent[opp]; ------------I don't know why this is here--------------------
...
---------------!!!I should
---------not use oll, orr here because they won't satisfy left[x]==oll. I need to actually use left[x].
-----need to change Composite.java too.
----need to remove the declaration of oll, orr variables!!!

fracLeft[opp] := fracLeft[opp] + 0.5;
fracRight[opp] := fracRight[opp] + 0.5;
packedCount[opp] := false;
// A future project: add indentation that is right to this translation!!
if (this==right[parent[this]]) {
fracRight[opp] := fracRight[opp]+0.5; ---------------why is this here?? Is it needed??-------------
call UnpackRight(this, lcc, parent[opp], opp);
if (this!=null) {
 fracCount[this] := fracCount[this] + 0.5;
}
packedRight[opp] := false;
fracCount[this] := fracCount[this]+0.5; --------this is 0.5+0.5 in the original!!!--------------
...
call PackRight(this, lcc, parent[opp], opp);------------lcc is lc+rc+1 in the original!!!!!!!!!!----------
if (this!=null) {
 fracCount[this] := fracCount[this] - 0.5;
}
packedRight[opp] := true;
	 call updateCountRec(parent[opp], count[opp], left[opp], this, count[left[opp]], lc+rc+1,this);
	 fracCount[this] := 0.0;---------------------this is parent[this] in the original!!!!!------------------
...
-------------------same questions that are in the first branch!!!!!!-----------------
fracLeft[opp] := fracLeft[opp]+0.5;
...
fracCount[this] := fracCount[this]+0.5;-------------this is 0.5+0.5 in the original--------------
	 call updateCount(lcc, ol, or, opp, lc, rc, count[opp], this);
fracCount[this] := fracCount[this];------------this is not needed!!not sure from where it came---------------------
...
//TO DO!! add a newline between procedures
--------------------OK until here---------------------
...

{
	 ...
	 assume (forall y:Ref :: (fracLeft[y] >= 0.0) );
	 assume (forall y:Ref :: (fracParent[y] >= 0.0) );
------------the original has assume (forall for all predicates!!!!------------
